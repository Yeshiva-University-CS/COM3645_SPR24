...
class Code_Collection extends Attribute {
    ...
    private final short maxStack;                                  // max stack space for method
    private final short maxLocals;                                 // max local variable index
    private final int codeLength;                                  // length of code block
    private final java.util.Vector code = new java.util.Vector();  // collection of code operations
    private final TryCatch_Collection trycatches;                  // try catch blocks
    //    (LineNumberTable, LocalVariableTable)
    private final Attribute_Collection attributes;                 // code attributes
            ...
            if (dispatch[opcode][3] == null) {
                ...
                try {
                    ...
                    try {
                        ...
                        myConstructor = newClass.getConstructor(ios.getClass(), pool.getClass(), int.class, int.class);
                    ...
                    myCode = (Code) myConstructor.newInstance(new Object[]
                            {ios, pool, Integer.valueOf(opcode), Integer.valueOf(pc)});
...
class Code_Branch extends Code {
    private final int branch;                 // branch offset for the operation (relative to current pc)
...
class Code_VarTable extends Code {
    private final int lvtIndex;               // index into the local variable table
...
class Code_Pool extends Code {
    private final int cptIndex;               // index into the constant pool table
    ...
    /*-----------------------------------------------------------------------
     ...
     -----------------------------------------------------------------------*/
    int browseFieldref() {
        if (pool.isFieldref(cptIndex)) return cptIndex;
        else return -1;
    }
    ...
    /*-----------------------------------------------------------------------
     ...
     -----------------------------------------------------------------------*/
    int browseMethodref() {
        if (pool.isMethodref(cptIndex)) return cptIndex;
        else return -1;
    }
    ...
    /*-----------------------------------------------------------------------
     ...
     -----------------------------------------------------------------------*/
    int browseInterfaceMethodref() {
        if (pool.isInterfaceMethodref(cptIndex)) return cptIndex;
        else return -1;
    }
}
...
class Code_bipush extends Code {
    private final int value;                  // value to push onto the stack
...
class Code_iinc extends Code_VarTable {
    private final int value;                  // increment value for the operation
...
class Code_invokeinterface extends Code_Pool {
    private final int count;                  // number of words to pop off the stack
...
class Code_lookupswitch extends Code {
    ...
    private final int lookupDefault;          // offset to the default handler
    private final int count;                  // number of match-branch pairs
    private final int[] match;                // case values to match int
    private final int[] branch;               // offset to the handler for the match
...
class Code_multianewarray extends Code_Pool {
    private final int dimensions;             // number of dimensions to allocate for the multidimensional array
...
class Code_newarray extends Code {
    /*-----------------------------------------------------------------------
     ...
     -----------------------------------------------------------------------*/
    private static final String[][] arrayTypeMap = {
            {"0", "unknown"},
            {"1", "unknown"},
            {"2", "unknown"},
            {"3", "unknown"},
    ...
    private final int arrayType;              // type of array to allocate
...
class Code_sipush extends Code {
    private final int value;                  // short value to be pushed on stack
...
class Code_tableswitch extends Code {
    ...
    private final int tableDefault;           // branch offset for the default handler
    private final int tableLow;               // low value for sequence of matches
    private final int tableHigh;              // high value for sequence of matches
    private final int count;                  // number of cases in switch (computed internally from low & high)
    private final int[] branch;               // branch offset for match
...
class Code_wide extends Code {
    private final int widecode;               // opcode for the wide extended instruction
    private final int lvtIndex;               // index into the local variable table
    private int value;                  // increment value for the wide iinc operation
    private final String wideDesc;            // description of the wide extended operation
