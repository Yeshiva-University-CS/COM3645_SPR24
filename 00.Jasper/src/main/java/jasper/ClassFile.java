...
public class ClassFile implements Serializable {
    ...
    public ClassFile(String name) {
        ...
        try {
            FIND:
            {
                ...
                for (String s : classPath) {
                    ...
                    if (f.exists()) {
                        if (f.isFile()) {
                            ...
                            f = new File(s + File.separatorChar + fileName);
                            if (f.isFile()) {
                ...
                // if we get to this point then throw file not found exception
                throw new FileNotFoundException(fileName);
            }

            // read the magic bytes - abort if not a class file
            // magic field of class file 0xcafebabe
            int magic = ios.readInt();
            ...
            int minorVersion = ios.readShort();
            // compiler major version number
            int majorVersion = ios.readShort();
    ...
    public static String pad(String s, int pad) {
        StringBuilder a = new StringBuilder(pad);
        a.append(" ".repeat(Math.max(0, pad)));
    ...
    public static void dump(DataInputStream ios, int length) {
        try {
            ...
            for (int i = 0, j = 0; i < length; i++) {
                ...
                if (s.length() < 2) s = "0" + s;
                System.out.print(s + " ");
                if ((val > 32) && (val < 128)) a.append((char) val);
                else a.append(' ');
                if (++j == 16) j = 0;
            }
            System.out.println();
    ...
    public String browseSourceFile() {
        return attributes.browseDeprecated() + attributes.browseSynthetic() + accessString() + attributes.browseSourceFile();
    }
    ...
    public String browseSuper() {
        if (superClass > 0) return pool.browseString(superClass);
        else return "";
    }
    ...
    private String[] getClassPath() {
        ...
        for (int i = 0; i < classPath.length(); i++) {
            if (classPath.charAt(i) == ';') {
                ...
                s.append(classPath.charAt(i));
            }
        }
        if (!s.toString().equals("")) x.add(s.toString());
