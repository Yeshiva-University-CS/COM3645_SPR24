...
public class Pcl4 {
    public static void main(String[] args) {
        if (args.length < 2) {
            System.out.println("Usage: Pcl4 -{execute | convert | compile} sourceFileName");
            System.exit(-1);
        }

        String operation = args[0];  // -execute, -convert, or -compile
        ...
        if (operation.equalsIgnoreCase("-execute")) {
            executeProgram(tree);
        } else if (operation.equalsIgnoreCase("-convert")) {
            System.out.println("Operation TBD: " + operation);
            System.exit(-1);
        } else if (operation.equalsIgnoreCase("-compile")) {
            System.out.println("Operation TBD: " + operation);
            System.exit(-1);
        } else {
            System.out.println("Invalid operation: " + operation);
            System.out.println("Usage: Simple -{scan | parse | execute} <sourceFileName>");
            System.exit(-1);
        }
    }
