...
public class Converter extends PascalBaseVisitor<Object> {
    ...
    public Object visitRoutineDefinition(
        ...
        PascalParser.ParametersContext paramsCtx;
        ...
        code.emit("(");
        if (paramsCtx != null) visit(paramsCtx);
        code.emitEnd(")");
        code.emitLine("{");
        code.indent();
    ...
    public Object visitParameterDeclarations(
        ...
        PascalParser.ParameterIdentifierListContext paramListCtx =
        ...
        for (PascalParser.ParameterIdentifierContext paramIdCtx :
                paramListCtx.parameterIdentifier()) {
            code.emit(currentSeparator);
            code.split(60);

            visit(typeCtx);
            code.emit(" " + paramIdCtx.entry.getName());

            if (paramType.getForm() == ARRAY) emitArraySpecifier(paramType);
            currentSeparator = ", ";
        }
    ...
    public Object visitCaseBranch(PascalParser.CaseBranchContext ctx) {
        ...
        if (listCtx != null) {
            ...
            for (PascalParser.CaseConstantContext constCtx :
                ...
                String txt = constCtx.getText().toLowerCase();
                if (constCtx.type == Predefined.stringType)
                    txt = txt.replace("'", "\"");
                code.emitLine("case " + txt + ":");
            }
